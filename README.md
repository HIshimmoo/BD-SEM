...
- Models per \((y,x)\) across four families:
  - `linear`: \( y = \beta_0 + \beta_1 x \)
  - `log10x`: \( y = \beta_0 + \beta_1 \log_{10} x \) (requires \(x>0\))
  - `reciprocalx`: \( y = \beta_0 + \beta_1 (1/x) \) (requires \(x\neq 0\))
  - `expY_linearx`: \( \ln y = \beta_0 + \beta_1 x \Rightarrow \hat y=\exp(\beta_0+\beta_1 x) \) (requires \(y>0\))
...
col1 = \(B_D\), col2 = \(\eta_{\mathrm{kin}}\) (V)
...
**Output**: `result_fit_kin_nls.xlsx` with sheets
- `params`: \(\hat\lambda_{\mathrm{kin}}\), SE, 95% CI; constants \(b, j_0, j\).
- `data_fit`: observed vs fitted, residuals.
- `metrics`: \(R^2\) and **LOO‑CV \(Q^2\)** computed on \(\eta_{\mathrm{kin}}\) scale.
...
col1 = \(B_D\), col2 = \(\eta_{\mathrm{RHF}}\) (V). Internally \(R_{\mathrm{HF}}=\eta_{\mathrm{RHF}}/j\).
...
**Output**: `result_fit_rhf_ols.xlsx` with sheets
...
**Delta‑method note (variance of \(\lambda\))**
\[
\lambda=\frac{a-R_{\mathrm{HF},0}}{b},\quad
\nabla\lambda=\begin{bmatrix}\partial\lambda/\partial a\\ \partial\lambda/\partial b\end{bmatrix}
=\begin{bmatrix}1/b\\ -(a-R_{\mathrm{HF},0})/b^2\end{bmatrix}
=\begin{bmatrix}1/b\\ -\lambda/b\end{bmatrix},
\quad \mathrm{Var}(\hat\lambda)\approx \nabla\lambda^\top\,\widehat{\mathrm{Cov}}(a,b)\,\nabla\lambda.
\]
```latex
\lambda=\frac{a-R_{\mathrm{HF},0}}{b},\quad
\nabla\lambda=\begin{bmatrix}1/b\\ -\lambda/b\end{bmatrix},\quad
...
col1 = \(B_D\), col2 = \(\eta_{\mathrm{RCL}}\) (V).
...
**Output**: `result_fit_rcl_nls.xlsx` with sheets
...
- `b` (Tafel slope, V/dec), `j0` (A/cm²), `j` (A/cm²), `alpha` (dimensionless), `R_HF0` (Ω·cm²).
...
- For NLS models (`fit_kin_nls.py`, `fit_rcl_nls.py`), ensure **\(B_D>0\)** and, where required, **\(y>0\)** (e.g., any log transforms inside the model).
...
- **Point estimate ± 95% CI**: look in the `params` sheet.
...
- **SciPy least_squares fails** → tweak initial guesses; check that all inputs are finite and positive where required.
- **Q² is NaN** → too few valid LOO predictions (e.g., after filtering), or denominator \(\sum_i (y_i-\bar y)^2 = 0\).

---

## Method references (equations for SI)
